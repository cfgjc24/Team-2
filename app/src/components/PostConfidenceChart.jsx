  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const querySnapshot = await getDocs(collection(firestore, 'student_data'));

        if (!querySnapshot.empty) {
          const dataFromFirebase = querySnapshot.docs.map(doc => doc.data());

          const postConfidenceArray = Array(8).fill(0);
          const moduleCount = {};
          ...
          dataFromFirebase.forEach(data => {
            ...
            if (moduleIndex >= 0 && moduleIndex < postConfidenceArray.length) {
              postConfidenceArray[moduleIndex] += data['confidence_after'];
              moduleCount[moduleIndex] = (moduleCount[moduleIndex] || 0) + 1;
            }
          });

          // Compute the average confidence for each module
          const averagedPostConfidenceArray = postConfidenceArray.map((sum, index) => {
          ...
          const chartData = {
            ...
            datasets: [{
              data: averagedPostConfidenceArray,
              backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(255, 159, 64, 0.2)',
                'rgba(255, 205, 86, 0.2)',
